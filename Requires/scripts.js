...
const $container_login = document.getElementById("container-login");
const $container_password = document.getElementById("container-password");
const $container_email = document.getElementById("container-email");
const $card_search = document.getElementById("card-search");
const $container_cart = document.getElementById("container-cart");
const $container_post = document.getElementById("container-post");
const $container_posts = document.getElementById("container-posts");
const $card_searchPost = document.getElementById("card-search-post");
const $card_search_postId = document.getElementById("card-search-postId");
...
window.login = function() {
    ...
    let inputPassword = $container_password.value;
    let inputEmail = $container_email.value;
    let userFound = false;
...
window.register = function() {
    ...
    if(!userFound && inputEmail !== "" && inputPassword !== ""){
        const newUser = {
            id : lastUser.id + 1,
            email: inputEmail,
...
window.renderListUser = function(){
    htmlListUser = "";
    let searchUsers = [];
    console.log($card_search);
    let inputSearch = "";
    if($card_search !== null){
        inputSearch = $card_search.value;
    }
    ...
    if(inputSearch === ""){
        for(let item of users){
            htmlListUser += `
        <tr>
                    <td class="p-4" style="width: 40px">
                        <div class="media align-items-center d-flex">
                            <div class="media-body" style="margin-left: 12px">
                                <a href="#" class="d-block text-dark" style="text-decoration: none">${item.id}</a>
                            </div>
                        </div>
                    </td>
        ...
        for(let user of users){
            ...
            if(user.email.includes(inputSearch) || user.first_name.includes(inputSearch) || user.last_name.includes(inputSearch)){
                searchUsers.push(user);
            }
        }

        for(let item of searchUsers){
            htmlListUser += `
        <tr>
                    <td class="p-4" style="width: 40px">
                        <div class="media align-items-center d-flex">
    ...
    if($container_cart !== null){
        $container_cart.innerHTML = htmlListUser;
    }
...
window.renderListPosts = function(){
    console.log("123")
    htmlPosts = "";
    for(let item of posts){
        let user = users.find(x=>x.id === item.user_id);
        htmlPosts += `
        <tr>
                    <td class="p-4" style="width: 40px">
                        <div class="media align-items-center d-flex">
    ...
    $container_posts.innerHTML = htmlPosts;
}

renderListPosts();

window.renderListPostsSearch = function(){
    let inputSearch = "";
    let newListPosts = [];
    inputSearch = $card_searchPost.value;
    let user = null;
    let userId = 0;
    user = users.find(x=>x.email === inputSearch);
    if(user !== null){
        userId = user.id
    }
    ...
    htmlPosts = "";
    for(let item of newListPosts){
        let userFound = users.find(x=>x.id === item.user_id);
        htmlPosts += `
        <tr>
                    <td class="p-4" style="width: 40px">
                        <div class="media align-items-center d-flex">
...
window.renderListPostsSearchById = function(){
    let inputSearch = 0;

    inputSearch = $card_search_postId.value;
    htmlPosts = "";

    for(let item of posts){

        if(item.id == inputSearch){
            let userFound = users.find(x=>x.id === item.user_id);
